...
int main() {
  ...
  std::cerr << c << a << b;
  Vector ab(a, b);
  Straight general(ab, a);
  Vector normal(general.a_, general.b_);
  Straight help(normal, c);
  Point x(Intersection(general, help));
  Vector h(c, x);
  std::cout << std::fixed << std::setprecision(6) << h.Size() << '\n';
  if (general.CheckRay(a, x, general.Guide())) {
    std::cout << h.Size() << '\n';
    if (general.CheckSection(a, x, b)) {
      std::cout << h.Size() << '\n';
    } else {
      Vector res(c, b);
      std::cout << res.Size() << '\n';
