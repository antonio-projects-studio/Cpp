...
bool Inside(const Point *point, const Point &c, int n) {
  Point res_2(100000000, c.y_ + 1);
  Straight res(c, res_2);
  int cnt = 0;
  for (int i = 0; i < n - 1; ++i) {
    Straight copy(point[i], point[i + 1]);
    if (CheckCrossing(copy, res, point[i], point[i + 1], c, res_2)) {
  ...
  Straight copy(point[n - 1], point[0]);
  if (CheckCrossing(copy, res, point[n - 1], point[0], c, res_2)) {
    if (copy.CheckStraight(c)) {
      return true;
    }
    ++cnt;
...
long double Square(const Point *point, int n) {
  int64_t sum_1 = 0, sum_2 = 0;
  for (int i = 0; i < n - 1; ++i) {
    sum_1 += point[i].x_ * point[i + 1].y_;
    sum_2 += point[i + 1].x_ * point[i].y_;
  }
  int64_t res_sum = sum_1 + point[n - 1].x_ * point[0].y_ - sum_2 -
                    point[0].x_ * point[n - 1].y_;
...
int main() {
  ...
  std::cout << std::fixed << std::setprecision(15) << Square(point, n);
  delete[] point;
  return 0;
}